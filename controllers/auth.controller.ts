...
export const registerUser = async (req: Request, res: Response) => {
	...
	try {
		...
		responseContent = {message: `User with username: ${newUser.Username} and email: ${newUser.Email} created` }
	} catch (error) {
...
export const login = async (req: Request, res: Response) => {
	...
	try {
		...
		const token = jwt.sign({ userid: credentialToLogin.idUser }, process.env.JWT_SECRET as string, { expiresIn: '1h' })
